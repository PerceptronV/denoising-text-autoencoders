...
parser.add_argument('--map-model', metavar='FILE',
                    help='path to mapping model checkpoint file')
...
if __name__ == '__main__':
    ...
    if args.arithmetic:
        ...
        fc = args.data
        sc = load_sent(fc)
        zc = torch.tensor(encode(sc)).to(device)
        print(f"zc shape: {zc.shape}")
        zd = torch.tensor(mappingModel(zc)).to(device)
        print(f"zd shape: {zd.shape}")
        sd = decode(zd)
        write_sent(sd, args.output)
